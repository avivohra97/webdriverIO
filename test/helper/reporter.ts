...
function addStep( loglevel: string, msg: string, toAllure = true, issueid = undefined) {
    ...
    try {
        if (loglevel === "info") logger.info(`[for info  level]: ${msg}`)
        if (loglevel === "debug") logger.debug(`[from debug level]: ${msg}`)
        if (loglevel === "warn") logger.warn(`[from warn level]: ${msg}`)
        if (loglevel === "error") {
            logger.error(`[from error level]: ${msg}`)
